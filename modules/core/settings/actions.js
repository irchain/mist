...
export function syncBuildConfig(key, value) {
  return {
    type: "[MAIN]:BUILD_CONFIG:SYNC", payload: { [key]: value }
  };
}
...
export function setLanguageOnMain(lang) {
  return dispatch => {
    ...
    try {
      ...
      dispatch({
        type: "[MAIN]:SET_LANGUAGE_ON_MAIN:SUCCESS", payload: { i18n: lang }
      });
    } catch (error) {
...
export function setLanguageOnClient(lang, browserWindow) {
  return dispatch => {
    ...
    try {
      browserWindow.webContents.executeJavaScript(`TAPi18n.setLanguage("${lang}");`);
      dispatch({
        type: "[MAIN]:SET_LANGUAGE_ON_CLIENT:SUCCESS", payload: { i18n: lang }
      });
    } catch (error) {
...
export function setAcceptLanguageHeader(lang, browserWindow) {
  return dispatch => {
    ...
    try {
      ...
      dispatch({
        type: "[MAIN]:SET_ACCEPT_LANGUAGE_HEADER:FAILURE", error
      });
    }
  };
}
...
export function getLanguage(event) {
  return (dispatch, getState) => {
    ...
    try {
      ...
      dispatch({
        type: "[MAIN]:GET_LANGUAGE:SUCCESS", payload: { i18n }
      });
    } catch (error) {
