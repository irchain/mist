...
exports.initializeListeners = function() {
  ...
  ipc.on("dbSync-removed", (event, args) => {
    ...
    item ? coll.remove(item) : log.error("Item not found in db", _id);
  });
...
const syncDataFromBackend = function(coll) {
  ...
  return new Promise((resolve, reject) => {
    const dataJson = ipc.sendSync("dbSync-reloadSync", { collName });
        ...
        record =>
          Tracker.afterFlush(() => {
            try {
              ...
              if (isString(record.redirect) && record.redirect.indexOf("//about:blank") > -1) record.redirect = null;
              record._id ? coll.upsert(record._id, record) : coll.insert(record);
            } catch (err) {
              console.error(err.toString());
            }
            done++;
            if (done >= dataJson.length) resolve();
          }));
    } catch (err) {
      reject(err);
    }
...
exports.frontendSyncInit = function(coll) {
  ...
  coll.onceSynced = new Promise((resolve) => syncDoneResolver = resolve);
