...
exports.run = () => {
  ...
  // check()
  //   .then(update => {
  //     if (update) showWindow({ sendData: { uiAction_checkUpdateDone: update } });
  //     store.dispatch({ type: "[MAIN]:UPDATE_CHECKER:FINISHED" });
  //   })
...
exports.runVisibly = () => {
  const wnd = showWindow({ sendData: "uiAction_checkUpdateInProgress" });
  wnd.on("ready", () => {
    check()
      .then(update => wnd.send({ uiAction_checkUpdateDone: update }))
      .catch(err => {
        log.error(err);
  ...
  return got("https://api.github.com/repos/happyuc-project/mist/releases", { timeout: 3000, json: true })
    .then(res => {
      ...
      if (semver.gt(latest.tag_name, Settings.appVersion)) {
        ...
        return {
          name: latest.name, version: latest.tag_name, url: latest.html_url
        };
      }
      log.info("App is up-to-date.");
    })
    .catch(err => log.error("Error checking for update", err));
};
