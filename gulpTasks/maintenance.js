...
gulp.task('update-nodes', cb => {
  const clientBinariesGhuc = clientBinaries.clients.Ghuc;
  const localGhucVersion = clientBinariesGhuc.version;
  ...
  // Return tag name (e.g. 'v1.5.0')
    .then(tagName => {
      const latestGhucVersion = tagName.match(/\d+\.\d+\.\d+/)[0];

      // Compare to current ghuc version in clientBinaries.json
      if (cmp(latestGhucVersion, localGhucVersion)) {
        ghuc.version = latestGhucVersion;

        // Query commit hash (first 8 characters)
        got(
          `https://api.github.com/repos/happyuc-project/happyuc-go/commits/${tagName}`,
          {json: true},
        ).then(response => {
          ...
          got(
            ...
            _.keys(ghuc.platforms).forEach(platform => {
              _.keys(ghuc.platforms[platform]).forEach(arch => {
                // Update URL
                let url = ghuc.platforms[platform][arch].download.url;
                url = url.replace(
                  /\d+\.\d+\.\d+-[a-z0-9]{8}/,
                  `${latestGhucVersion}-${hash}`,
                );
                ghuc.platforms[platform][arch].download.url = url;

                // Update bin name (path in archive)
                let bin = ghuc.platforms[platform][arch].download.bin;
                bin = bin.replace(
                  /\d+\.\d+\.\d+-[a-z0-9]{8}/,
                  `${latestGhucVersion}-${hash}`,
                );
                ghuc.platforms[platform][arch].download.bin = bin;

                // Update expected sanity-command version output
                ghuc.platforms[platform][
                  arch
                  ].commands.sanity.output[1] = String(latestGhucVersion);
                ...
                blobs.forEach(blob => {
                  if (
                    String(blob.Name) ===
                    _.last(
                      ghuc.platforms[platform][arch].download.url.split('/'),
                    )
                  ) {
