...
if (global.mode === "wallet") {
  ...
  global.interfaceAppUrl    = Settings.inProductionMode ? `file://${__dirname}/interface/wallet/index.html` : "http://localhost:3050";
  global.interfacePopupsUrl = Settings.inProductionMode ? `file://${__dirname}/interface/index.html` : "http://localhost:3000";
} else {
  // - MIST
  log.info("Starting in Mist mode");

  let url = Settings.inProductionMode ? `file://${__dirname}/interface/index.html` : "http://localhost:3000";
  if (Settings.cli.resetTabs) url += "?reset-tabs=true";
...
store.dispatch({
  type: "[MAIN]:PROTOCOL:REGISTER", payload: { protocol: "bzz" }
});
...
function enableSwarmProtocol() {
  protocol.registerHttpProtocol("bzz", (request, callback) => {
    if ([SwarmState.Disabling, SwarmState.Disabled].includes(store.getState().settings.swarmState)) {
      ...
      callback({ error });
      store.dispatch({
        type: "[MAIN]:PROTOCOL:ERROR", payload: { protocol: "bzz", error }
      });
      return;
    }
    ...
    store.dispatch({
      type: "[MAIN]:PROTOCOL:REQUEST", payload: { protocol: "bzz" }
    });
  }, error => { if (error) log.error(error); });
}
...
async function kickStart() {
  ...
  if (!huappyucNode.isIpcConnected) throw new Error("Either the node didn't start or IPC socket failed to connect.");
  ...
  log.info("finished show splash window");

  if (!Settings.inAutoTestMode) await handleNodeSync();

  log.info("finished handle node sync");
...
function initializeKickStartListeners() {
  ...
  huappyucNode.on("state", (state, stateAsText) => {
    Windows.broadcast("uiAction_nodeStatus",
      stateAsText,
...
function initializeTabs() {
  const Tabs       = global.db.getCollection("UI_tabs"),
        sortedTabs = Tabs.getDynamicView("sorted_tabs") || Tabs.addDynamicView("sorted_tabs");
